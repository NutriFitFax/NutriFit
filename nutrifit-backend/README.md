...
| GET    | `/profile`                 | Read the current user's profile          |
...
`/db-health` and `/profile` are backed by Supabase Postgres (project `pyfpxlhmqjdorsqhbtyh`, eu-west-1) when `SUPABASE_DB_URL`, `SUPABASE_DB_USER`, and `SUPABASE_DB_PASSWORD` are set. The backend creates the `user_profiles` table on startup when the database is configured.
...
curl -H "X-User-Id: demo-user" http://127.0.0.1:8000/profile
